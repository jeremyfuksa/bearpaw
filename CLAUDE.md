...
| Cleared-row styling clears once the upload lands | [`frontend/src/app/components/views/ChannelsTab.tsx`](frontend/src/app/components/views/ChannelsTab.tsx) `isClearPending` in the row-render loop | `frontend/src/app/components/views/__tests__/ChannelsTab.test.tsx :: keeps a staged clear amber only while it is genuinely pending` | The amber "staged clear" treatment (#272) was gated on `isCleared` alone — a pure value test (`draft.frequency` parses to 0). But a successful upload REBUILDS the draft from the written channel rather than deleting it, so a genuinely-cleared channel keeps a zero-frequency draft for the life of the session; the row stayed amber and flashing forever after the write. Gate on `isCleared && isPending` so the styling tracks *staged* state, not "is zero". Same stale-draft trap as the `isPending` (#195) and priority/lockout (#206) guards in the same file — a draft can outlive the change it represented. |
