...
export const mockApiResponses = {
  ...
  } satisfies LockoutsResponse,
  squelch: { level: 5 },
  config: {
    firmware: '1.0.12',
    squelch: { level: 5 },
    backlight: { event: 'AO' } satisfies BacklightSettings,
    battery: { charge_time: 10 } satisfies BatterySettings,
    key_beep: { level: 2, lock: false },
    priority: { mode: 1 } satisfies PrioritySettings,
    search: { delay: 0, code_search: false } satisfies SearchSettings,
    close_call: {
      mode: 0,
      alert_beep: true,
      alert_light: true,
  ...
  keyBeep: { level: 2, lock: false } satisfies KeyBeepSettings,
  priority: { mode: 1 },
  ...
  closeCall: {
    ...
    band: [true, true, true, true, true],
    lockout: true,
  } satisfies CloseCallSettings,
  serviceSearch: {
    groups: [true, false, true, false, true, false, true, false],
  } satisfies ServiceSearchSettings,
  customSearch: {
    groups: [true, false, true, false, true, false, true, false],
  } satisfies CustomSearchSettings,
  customSearchRange: { index: 1, lower: 140, upper: 149 } satisfies CustomSearchRange,
  weather: { priority: true } satisfies WeatherSettings,
  contrast: { level: 8 } satisfies ContrastSettings,
