...
import { mockApiResponse, mockFetch, mockFetchError, mockFetchNetworkError, resetMockFetch } from "../../test/utils";
...
describe("ScannerAPIClient", () => {
  ...
  describe("settings", () => {
    ...
    it("should set close call settings successfully", async () => {
      mockFetch({});
      await expect(client.setCloseCallSettings(mockApiResponses.closeCall)).resolves.toBeUndefined();
    });
