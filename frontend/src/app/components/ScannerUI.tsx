...
const CONTROL_BUTTON_CLASSES =
  'inline-flex items-center justify-center rounded-scanner-xs border border-[rgba(28,31,38,0.8)] px-[clamp(4px,1.5cqmin,12px)] py-[clamp(2px,0.6cqmin,4px)] font-display font-medium text-[clamp(9px,2.2cqmin,14px)] leading-none text-[rgba(28,31,38,0.9)] transition-colors hover:bg-[rgba(28,31,38,0.1)] active:translate-y-[1px]';
...
function ScannerControls({
  ...
  return (
    <div className="flex shrink-0 items-center gap-[clamp(4px,1.2cqmin,12px)]">
      <Popover>
...
function DisplaySignalBars({ strength }: { strength: number }) {
  return (
    <div className="flex shrink-0 items-end gap-[clamp(1px,0.4cqmin,3px)]">
      {[1, 2, 3, 4, 5].map((bar) => (
        <span
          key={bar}
          className={cn(
            'h-[clamp(8px,2.6cqmin,20px)] w-[clamp(2px,0.8cqmin,6px)] rounded-scanner-xs',
...
export function ScannerDisplay({
  ...
  return (
    ...
    >
      <div className="relative flex h-full w-full flex-col gap-[clamp(4px,1.5cqmin,12px)] px-[clamp(8px,2.8cqmin,24px)] py-[clamp(6px,2.2cqmin,20px)]">
        {/* Top row — USB / error indicator on the left, mini controls on the right */}
        <div className="flex w-full items-center justify-between">
          <div className="relative shrink-0 size-[clamp(12px,3cqmin,24px)] text-[rgba(28,31,38,0.9)]">
            {isError ? (
              errorType === 'socket' ? (
                <SocketErrorIcon />
              ) : (
        ...
        <div className="flex flex-1 min-h-0 flex-col justify-center gap-[clamp(4px,1.2cqmin,12px)] border-y border-[rgba(28,31,38,0.6)] py-[clamp(4px,1.5cqmin,16px)]">
          <p
            className={cn(
              'font-display font-extrabold text-[clamp(24px,9cqmin,72px)] leading-[1.05] tracking-tight text-[rgba(28,31,39,0.7)] truncate',
          ...
          <div className="flex w-full items-center justify-between gap-2">
            <p className="font-display font-normal text-[clamp(12px,4.5cqmin,28px)] leading-tight text-[rgba(28,31,39,0.6)] truncate">
...
export function BankControls({ activeBanks, onToggleBank }: BankControlsProps) {
  ...
  return (
    <div className="flex w-full items-center justify-between gap-[clamp(2px,0.6cqmin,8px)]">
      {banks.map((bank, index) => {
        ...
        return (
          <button
            ...
            className={cn(
              'inline-flex h-[clamp(18px,5cqmin,40px)] flex-1 items-center justify-center rounded-scanner-xs font-display font-medium text-[clamp(11px,3cqmin,18px)] leading-none transition-colors',
              isActive
